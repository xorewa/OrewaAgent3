...
function readHermesModel(): { model: string; source: 'env' | 'config' | 'fallback' } {
  try {
    const configPath = path.join(os.homedir(), '.orewa3', 'config.yaml');
...
function readConfiguredHermesPath(): { value: string; workspaceOverrideIgnored: boolean } {
  const hermesConfig = vscode.workspace.getConfiguration('orewa3');
...
function resolveHermesBinary(configuredPath: string): string {
  ...
  if (hermesPath !== 'hermes' && !path.isAbsolute(hermesPath)) {
    throw new Error('orewa3.path must be an absolute path or the default "hermes" value');
  }
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  outputChannel = vscode.window.createOutputChannel('Orewa Agent 3');
  ...
  // OrewaAgent3: run the shared `hermes` agent binary against an isolated home so
  ...
  const orewaHome = path.join(os.homedir(), '.orewa3');
  ...
  // Commands
  context.subscriptions.push(
    vscode.commands.registerCommand('orewa3.openChat', async () => {
      outputChannel.appendLine('[ui] open chat');
      await vscode.commands.executeCommand('orewa3.chatView.focus');
      await ensureConnected();
    }),

    vscode.commands.registerCommand('orewa3.newSession', () => {
